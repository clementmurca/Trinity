...
const InvoiceTemplateAdmin = () => {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      formData.append('file', pdfBlob, `Facture-${invoiceId}.pdf`)

      const token = localStorage.getItem('token')
      await axios.post(`${API_URL}/invoices/upload/${invoiceId}`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      setMessage('PDF send with success')
    } catch (error) {
      console.error('error for sending PDF', error)
      setMessage('Not send')
    }
  }
  ...
  return (
    <Card className="shadow-lg border border-gray-200 rounded-lg">
      ...
      <div className="px-6 my-4 flex items-center space-x-4">
        <Button onClick={generatePDF}>Save backend</Button>
...
export default InvoiceTemplateAdmin
